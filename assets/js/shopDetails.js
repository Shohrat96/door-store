...
const getSingleProduct = async () => {
  ...
  const productImagesContainerNav = document.querySelector(
    ".fz-product-details__img-nav"
  );
  ...
  productCategoryEl.textContent = categories.find(
...
const renderAddToWishlistBtn = () => {
  ...
  const template = `
    <button class="fz-product-details__add-to-wishlist ${
      isInWishList ? "fz-product-details__add-to-wishlist-active" : ""
    }">
        <i class="${isInWishList ? "fa" : "fa-light"} fa-heart"></i>
        <span style="margin-left: 4px;"">${
          isInWishList ? "Bəyəndiklərimdən çıxart" : "Bəyəndiklərimə əlavə et"
        }</span>
    </button>`;
  ...
  tempElement.firstChild.addEventListener("click", () => {
    if (isInWishList) {
      ...
      localStorage.setItem(
        "wishlist",
        JSON.stringify([...wishlistItems, productMatch])
      );
    }
