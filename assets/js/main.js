...
const productsContainerRow = document.querySelector(".fz-inner-products-container .row");
const singleProductTemplate = (imageUrl, title, price, id) => `
...
<div class="fz-single-product__img">
    <img src="assets/images/roomDoors/${imageUrl}" alt="Product Image">
...
const sortProducts = (sortOption, products) => {
    let sortedProducts;
    if (sortOption === 'price-asc') { // price-asc | price-desc
...
const getBetweenPrice = (products, priceRange) => {
    const filteredProducts = products.filter(item => {
        ...
        if (parseInt(item.price) >= priceRange[0] && parseInt(item.price) <= priceRange[1] ) return item
    })
...
const getProducts = async () => {
    ...
    products.forEach((item) => {
        ...
        newEl.innerHTML = singleProductTemplate(item.images[0], item.title, item.price, item.id).trim()
